...
namespace ModU3DToolkit.VFX
{
	public class Polyline
	{
        ...
        protected Vector3 lastPointPosition = Vector3.zero;
        #endregion
        ...
        {
            var curBiNormal = Vector3.Cross(curPt.tangent, curPt.normal).normalized;
            var nextBiNormal = Vector3.Cross(nextPt.tangent, nextPt.normal).normalized;
        ...
        public void Reset(Point startPoint)
        {
            ...
            var biNormal = Vector3.Cross(startPoint.tangent, startPoint.normal).normalized;
        ...
        public bool AddPoint(Point newPoint)
        {
            ...
            if (!newPoint.isFirst)
            {
                if (minDistBetweenPoints > 0.0f && Vector3.Distance(lastPointPosition, newPoint.point) < minDistBetweenPoints)
                {
                    return false;
                }
            ...
            lastPointTimeStamp = now - Mathf.Min(minTimeBetweenPoints, diff - minTimeBetweenPoints);
